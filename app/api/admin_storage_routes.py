...
async function readResponse(r){
  ...
  catch(e) { return {success:false,error:`השרת החזיר תשובה שאינה JSON (HTTP ${r.status})`,details:text.slice(0,1200)}; }
}
async function send(url){
  const f=zip.files[0];
  if(!f){alert('בחר קובץ ZIP');return null;}
  const fd=new FormData(); fd.append('backup',f,f.name); status.className=''; status.textContent='מעבד...';
  const r=await fetch(url,{method:'POST',body:fd,credentials:'same-origin'});
  const j=await readResponse(r); out.textContent=JSON.stringify(j,null,2);
  if(!r.ok) throw new Error(j.error||`הפעולה נכשלה (HTTP ${r.status})`);
  return j;
}
document.getElementById('preview').onclick=async()=>{restore.disabled=true;lastPlan=null;try{const j=await send('/api/admin/storage/recovery/preview');if(j&&j.success){lastPlan=j;restore.disabled=!(j.counts&&j.counts.matched>0);status.className='ok';status.textContent=`נמצאו ${j.counts.matched} מסמכים לשחזור.`}else{status.className='err';status.textContent=(j&&j.error)||'ה-Preview נכשל';}}catch(e){status.className='err';status.textContent=e.message}};
restore.onclick=async()=>{if(!lastPlan)return;if(!confirm('לבצע שחזור של כל ההתאמות הבטוחות?'))return;restore.disabled=true;try{const j=await send('/api/admin/storage/recovery/restore');status.className=j&&j.success?'ok':'err';status.textContent=j&&j.success?'השחזור הושלם':'השחזור הושלם חלקית — בדוק את הדוח';}catch(e){status.className='err';status.textContent=e.message}finally{restore.disabled=false}};
