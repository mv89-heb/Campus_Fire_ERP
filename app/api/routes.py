...
def dashboard():
    try:
        zones = Zone.query.all(); requirements_processed = []; alerts = {'expired': [], 'critical_14': [], 'warning_30': []}; valid_count = 0; total_reqs = 0
        for zone in zones:
        ...
        current_app.logger.exception('Dashboard Error'); return jsonify({'error': 'אירעה שגיאה פנימית'}), 500


@main_bp.route('/api/documents/upload_bulk', methods=['POST'])
def upload_bulk():
    processed = 0; skipped_duplicates = 0; errors = []; files = request.files.getlist('file'); max_pdf_bytes = current_app.config['MAX_PDF_BYTES']
    for f in files:
        ...
        try:
            ...
            if result is not None: processed += 1
            else: skipped_duplicates += 1
        except Exception as e:
            current_app.logger.warning(f'Upload rejected/failed for {f.filename}: {e}'); errors.append({'file': f.filename, 'reason': str(e)})
    success = processed > 0 or (skipped_duplicates > 0 and not errors)
    return jsonify({'success': success, 'processed': processed, 'skipped_duplicates': skipped_duplicates, 'errors': errors, 'total_files': len(files)})
...
def system_health():
    ...
    try:
        zone_count = Zone.query.count(); result['db_connected'] = True; result['zones_seeded'] = zone_count > 0; result['zone_count'] = zone_count
    ...
    try:
        ...
        os.remove(test_path); result['upload_folder_writable'] = True
    except Exception as e: result['upload_folder_error'] = str(e)
    ...
    if result['supabase_configured']:
        connected, storage_error = storage.check_connection(); result['storage_connected'] = connected
        if storage_error: result['storage_error'] = storage_error
    else:
        result['storage_connected'] = False; result['storage_note'] = 'Supabase לא מוגדר - מסמכים חדשים יישמרו מקומית'
...
def outlook():
    ...
    try:
        import win32com.client as win32; import pythoncom
        pythoncom.CoInitialize()
        try:
            outlook = win32.Dispatch('outlook.application'); mail = outlook.CreateItem(0); mail.To = 'tservice@102.gov.il'; mail.Subject = 'הגשת מסמכי רישוי'; mail.Body = 'שלום רב,\nמצ"ב מסמכים מעודכנים.'; mail.Display(False); return jsonify({'success': True})
        finally: pythoncom.CoUninitialize()
    except Exception:
        current_app.logger.exception('Outlook integration failed'); return jsonify({'error': 'שגיאת Outlook'}), 500
